...
import wighawag.gpu.GPUProgram;
import wighawag.gpu.TextureBatch;
import wighawag.gpu.TextureData;

class TexturedQuadProgram implements GPUProgram{

	public var batches : StringMap<TextureBatch>;
	...
	public function setContext(context3D : Context3D) : Void{
        ...
        var fragShader = // - not on desktop ? 'precision mediump float;' +
            ...
            //TODO save the glslProgram statically to be shared acrros TexturedQuadProgram instances
            // one glslProgram per context3d. hashMap ?
...
class Batch{
    ...
    public function draw(context3D : Context3D, glslProgram : GLSLProgram, projectionMatrix : Matrix3D) : Void{
        ...
        glslProgram.setVertexBufferAt("position",vertexBuffer, 0, flash.display3D.Context3DVertexBufferFormat.FLOAT_2);
        glslProgram.setVertexBufferAt("uv",vertexBuffer, 2, flash.display3D.Context3DVertexBufferFormat.FLOAT_2);
        glslProgram.setVertexBufferAt("alpha",vertexBuffer, 4, flash.display3D.Context3DVertexBufferFormat.FLOAT_1);
